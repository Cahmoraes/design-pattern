...
test('Não deve criar um financiamento imobiliário com prazo superior a 420 meses', () => {
  expect(() => MortgageLoan.create(100000, 10000, 450)).toThrow(
    new Error('The maximum number installments for mortgage loan is 420'),
  )
})
...
test('Não deve criar um financiamento veicular superior a 60 meses', () => {
  expect(() => CarLoan.create(100000, 10000, 72)).toThrow(
    new Error('The maximum number for installments for car loan is 60'),
  )
})
